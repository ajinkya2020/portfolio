...
function ProjectsComponent() {
  return (
    <div className="projects-container mt-20 py-5">
      ...
      <div>
        Explore my projects and witness the fusion of technology and genius.
      </div>
      <div className="flex flex-wrap justify-center">
        {PROJECT_LIST.map((skill, index) => (
          <ProjectCardComponent
            key={index}
            cardImage={skill.image}
            title={skill.title}
            desc={skill.desc}
            skills={skill.skills}
            projectLink={skill.projectLink}
            sourceCodeLink={skill.sourceCodeLink}
          ></ProjectCardComponent>
        ))}
      </div>
    </div>
...
function ProjectCardComponent(props: ProjectCardComponentProps) {
  return (
      ...
      <div className="flip-card-inner rounded-lg">
        ...
        <div className="flip-card-back py-40 px-28 rounded-lg">
          <div>
            ...
            <button className={`${!props.projectLink && 'cursor-not-allowed'} mt-10 me-2 p-2`}><a href={props.projectLink} target="_blank" rel='noopener noreferrer'>Live Preview</a></button>
            <button className={`${!props.sourceCodeLink && 'cursor-not-allowed'} mt-5 p-2`}><a href={props.sourceCodeLink} target="_blank" rel='noopener noreferrer'>Source Code</a></button>
          </div>
        </div>
      </div>
    </div>
...
export function ProjectSkill(props: { skill: string }) {
  return (
